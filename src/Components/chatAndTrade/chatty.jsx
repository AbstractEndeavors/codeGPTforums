...
import NewPostForm from './../Forum/Posts/NewPostForm/NewPostForm';
...
function ChattyApp() {
  ...
  async function SendToNew(recieved) {
    return(recieved);
  }
  async function handleClick(inIt:string,user:string) {
    ...
    let data = strFyIt(ifInJsOg({ "prompt": prompt + "\n" + inIt + "\n", "user": user }, "chat"));
    try {
